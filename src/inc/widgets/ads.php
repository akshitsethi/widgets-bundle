...
namespace AkshitSethi\Plugins\WidgetsBundle\Widgets {
	...
	class Ads extends WP_Widget {
		...
		?>
			...
			<div class="as-wb-ads-upload">
				<span class="as-wb-ads-preview">
				...
				<input class="as-wb-ads-url" id="<?php echo esc_attr( $this->get_field_id( 'image' ) ); ?>" name="<?php echo esc_attr( $this->get_field_name( 'image' ) ); ?>" type="hidden" value="<?php echo esc_attr( $instance['image'] ); ?>" />
				<button class="button" id="as-wb-ads-btn"><?php esc_html_e( 'Select Image', 'widgets-bundle' ); ?></button>

				<span class="as-wb-ads-append">
					<?php if ( ! empty( $instance['image'] ) ) : ?>
						<a href="javascript:;" id="as-wb-ads-remove"><?php esc_html_e( 'Remove', 'widgets-bundle' ); ?></a>
					<?php endif; ?>
				</span>
			</div>
